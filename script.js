    ...
    function initializeControls() {
        ...
        // Initialize solution button
        document.getElementById('solve-btn').addEventListener('click', findSolution);
    ...
    // Solution finding functionality
    function findSolution() {
        ...
        isSolving = true;
        document.getElementById('solve-btn').textContent = 'Anulează';
        document.getElementById('solve-feedback').textContent = 'Așteptați...';
        
        // Start with 1 day trial
        trySolution(9, 1000);
    }

    function cancelSolution() {
        isSolving = false;
        document.getElementById('solve-btn').textContent = 'Rezolvă';
        document.getElementById('solve-feedback').textContent = 'Anulat de utilizator';
    ...
    function trySolution(daysToToggle, timeout) {
        if (!isSolving) return;
        ...
        function attempt() {
		    ...
		    if (daysToToggle > 1) {
                    ...
                    isSolving = false;
                    document.getElementById('solve-btn').textContent = 'Rezolvă';
                    document.getElementById('solve-feedback').textContent = 'Nu s-a găsit nicio soluție';
                }
                return;
            }
            
            // Generate random days to toggle
            const workDays = [];
            for (let day = 1; day <= daysInDecember; day++) {
                const isHoliday = decemberHolidays.includes(day) || saturdays.includes(day) || sundays.includes(day);
                const isWorkDay = ((day + 7 - tura) % 4 < 2);
		...
		daysToChange.push(day);
            ...
            if (newTotalHours === goalHours) {
                // Found a solution!
                isSolving = false;
                document.getElementById('solve-btn').textContent = 'Rezolvă';
                document.getElementById('solve-feedback').textContent = `Soluție găsită cu ${daysToToggle} zile modificate`;
                
                // Apply the solution
                currentSolution = daysToChange;
                applySolution(daysToChange);
                return;
            }
            
            // Try next attempt
            solutionTimeout = setTimeout(attempt, 10);
        }
        
        attempt();
    }

    function applySolution(daysToChange) {
        daysToChange.forEach(day => {
            const dayElement = document.querySelector(`.planner-calendar .day:nth-child(${day})`);
            toggleDayStatus(dayElement, day);
            dayElement.classList.add("solution");
        });
    ...
    function resetSolution() {
        // Remove solution highlighting
        document.querySelectorAll('.planner-calendar .day.solution').forEach(day => {
            day.classList.remove("solution");
        });
        
        document.getElementById('solve-feedback').textContent = '';
